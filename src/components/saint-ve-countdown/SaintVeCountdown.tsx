...
function createStVDate(today: Date) {
  ...
  const normalDate =
    today.getTime() > thisYearNormalEndDate.getTime()
      ? new Date(year + 1, november, day, beginHour)
      : thisYearNormalStartDate;
  const saturday = 0;
  const sunday = 6;
...
export default function SaintVeCountdown() {
  ...
  const diff =
    now.getTime() >= nextSaintV.startDate.getTime() &&
    now.getTime() <= nextSaintV.endDate.getTime()
      ? { days: 0, hours: 0, minutes: 0, seconds: 0 }
      : calcDifferenceInDates(now, nextSaintV.startDate);
    ...
    if (Object.values(remainders).some((value) => value > 0)) {
      const interval = setInterval(() => {
        setRemainders((prev) => {
          ...
          const diff =
            now.getTime() >= newStV.startDate.getTime() &&
            now.getTime() <= newStV.endDate.getTime()
              ? {
                  days: 0,
                  hours: 0,
                  minutes: 0,
  ...
  return (
    <>
      <canvas ref={upperCanvasRef}></canvas>
      <canvas ref={lowerCanvasRef}></canvas>
    </>
  );
}
